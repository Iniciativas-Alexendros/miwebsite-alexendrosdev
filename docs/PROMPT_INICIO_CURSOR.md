...
- **Objetivos:** Prompt de sesión para Agent mode: mantenimiento del MVP, no greenfield.
- **Estructura:** Rol → contexto → modo por defecto → prioridades → entrega.
- **Contenido a integrar según contexto:** Conserva exclusiones (pricing, CMS, Google Fonts). Actualiza solo si cambia el contrato de CI o deploy.
...
3. Verifica con `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm build`, `pnpm smoke` y e2e/axe cuando toque UI o a11y (opt-in).
